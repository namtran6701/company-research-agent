  ...
  return (
    ...
    >
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${
        ...
        {(() => {
          const displayedCategories = ['company', 'industry', 'auditor', 'news'] as const;
          return (
      ...
      {!isExpanded && (
        <div className="mt-2 text-sm text-gray-600">
          {(() => {
            const displayed = ['company', 'industry', 'auditor', 'news'] as const;
            const completed = displayed.filter((c) => briefingStatus[c]).length;
            return `${completed} of ${displayed.length} briefings completed`;
          })()}
        </div>
      )}
    </div>
